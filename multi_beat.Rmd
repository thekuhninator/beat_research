...
hvgs_kbet_plot <- function(boxplot_data, output_dir, output_name)
{
  # create the output file
  file_name <- paste(output_name, '_grouped_boxplot.png')
  output_file_path <- file.path(output_dir, file_name)
...
kbet_plot <- data.frame("kbet_acceptance" = numeric(), "dataset_name" = character(), "hvgs_retained" = numeric())
kbet_plot
...
for (dataset_name in datasets)
{
  ...
  kbet_plot[nrow(kbet_plot) + 1,] = new_data
}
...
```{r}
# i don't believe these are the actual accepted rates i think that's in the reuslts object from kbet
g <- ggplot(kbet_plot, aes(x=as.numeric(kbet_acceptance), y = as.numeric(hvgs_retained), color = dataset_name)) + 
  geom_point() + 
...
#plot(kbet_plot$kbet_acceptance, kbet_plot$hvgs_retained, main="KBET vs HVGs #Plot",
